...
type Partner = { id: string; title: string; imageUrl: string };
...
export async function GET(
  ...
  const res = await s3Service.loadJsonData('bossxd-partners', { partners: [] });
  const partners = ((res.data as { partners: Partner[] })?.partners) || [];
  const partner = partners.find((s) => s.id === id);

  if (!partner?.imageUrl) {
    return NextResponse.json({ error: 'Nie ma takiego zdjęcia' }, { status: 404 });
  }

  const key = decodeURIComponent(new URL(partner.imageUrl).pathname.replace(/^\//, ''));
