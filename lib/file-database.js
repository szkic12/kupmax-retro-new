...
// Save database with proper locking and atomic write
const saveDatabase = (data) => {
  if (typeof window !== 'undefined') {
    ...
    try {
      ...
      console.log(`[FileDatabase] Saving ${data.files.length} files to ${dbPath}`);
      ...
      console.log(`[FileDatabase] Successfully saved database with ${data.files.length} files`);
    } catch (error) {
      console.error('[FileDatabase] Error saving server database:', error);
...
export const FileDatabase = {
  ...
  addFiles: async (filesData) => {
    ...
    if (typeof window !== 'undefined') {
      return filesData.map(fd => FileDatabase.addFile(fd));
    }
